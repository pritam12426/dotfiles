...
// ----- internal macros -----------------------------------------------------
#define S(x, n)                                                  \
	(((((uint32_t) (x) & 0xFFFFFFFFUL) >> (uint32_t) ((n) & 31)) \
	  | ((uint32_t) (x) << (uint32_t) ((32 - ((n) & 31)) & 31))) \
	 & 0xFFFFFFFFUL)
...
#define SHA256_COMPRESS(buff)                                               \
	...
	for (int i = 0; i < 8; i++)                                             \
		sha256_state[i] = sha256_state[i] + S[i];
...
CRYPTOSHA256DEF int crypto_sha256(unsigned char out[32], const unsigned char *in, size_t len)
{
	...
	// copy remaining bytes (guarded to avoid NULL + 0 memcpy)
	if (len > 0) {
